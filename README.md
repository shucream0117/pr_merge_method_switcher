...
- **バナーの外枠に `margin` ショートハンドを使わない**。旧UI向けのフォールバックでは
  マージボックスの左余白クラス(`tmp-ml-md-6` など)を外枠に写して位置を合わせるため、
  `margin: 8px 0 0` のような指定は写した左マージンを打ち消してしまう。
  縦方向だけを個別に指定する
- バナーの挿入位置は `insertBanner()` の1箇所だけで決めている。マージボタンを含む枠
  (`mergebox-border-container`)の直下に置く。位置を戻したいときはコミット
  「バナーをマージボタンの下へ移す」を revert すればよい
