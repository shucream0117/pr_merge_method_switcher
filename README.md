...
マージボックスの選択状態を自動で切り替える。何をしたかはマージボタンの直下に
バナーで表示する。バナーはマージボックスの枠の中に置き、ボタン行との間に区切り線を
引くだけにして、マージボックスの一部として見えるようにしている。
...
- バナーの挿入位置は `insertBanner()` の1箇所だけで決めている。マージボタンが入っている
  区画(`mergebox-border-container` の中でマージボタンを含む直下の子)の末尾に追加し、
  枠線も背景もマージボックスのものを使う(`.pmms-banner--fused`)。位置や見た目を戻したい
  ときは、コミット「バナーをマージボタンの下へ移す」「バナーをマージボックスに融合させる」
  を revert すればよい
- 区画が見つからない場合(旧UI)は独立した枠としてマージボックスの直後に置く
  (`.pmms-banner--standalone`)。このときはマージボックスの左余白クラス(`tmp-ml-md-6` など)
  を外枠に写して位置を合わせるため、**`margin` ショートハンドを使わない**。
  `margin: 8px 0 0` のような指定は写した左マージンを打ち消してしまう
- React が描画している区画に要素を足すため、再描画で消される可能性がある。消えても
  MutationObserver が拾って入れ直すので、実害は一瞬の消失だけ
